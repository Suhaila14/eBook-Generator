...
const EditorPage = () => {
  ...
  //AI Modal State
  const [isOutlineModalOpen, setIsOutineModalOpen] = useState(false);
  ...
  /* const handleExportPDF = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        ...
        { responseType: "blob" }, // This will handle response as file
      );

      //this converts raw pdf data to a pdf file object (blob)
      //Blob = Put the PDF into a proper file folder.
      //createObjectURL = Give that folder an address.
      //Use the address to open or download the file
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url; // creating <a> tag and set it as download url
      link.setAttribute("download", `${book.title}.pdf`); // download the file and save it as the respective title.pdf
      document.body.appendChild(link); //Add this link element to the webpage.
      link.click();
      link.parentNode.removeChild(link); //Remove the temporary download link from the webpage
      window.URL.revokeObjectURL(url); //removing the url after download happens
      toast.dismiss();
    ...
    <>
      <div className="flex bg-slate-50 font-sans relative min-h-screen">
        ...
        {isSidebarOpen && (
          ...
          >
            <div
              className="fixed inset-0 bg-black/30 bg-opacity-75"
              aria-hidden="true"
            ...
            <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white">
              <div className="absolute top-0 right-0 -mr-12 pt-2">
                <button
                  type="button"
                  className="ml-1 flex items-center items-center justify-center h-10 w-10 rounded-full focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
                  onClick={() => setIsSidebarOpen(false)}
                >
            ...
            <div className="flex-shrink w-14" aria-hidden="true"></div>
          </div>
        )}
        ...
        <main className="flex-1 h-full flex flex-col">
          <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-sm border-b border-slate-200 p-3 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:flex space-x-1 bg-slate-100 p-1 rounded-lg">
                ...
                <button
                  ...
                  className={`flex items-center justify-center flex py-2 px-4 text-sm font-medium rounded-md transition-colors duration-200 whitespace-nowrap${
                    activeTab === "details"
                      ? "bg-white text-slate-800 shadow-sm"
                      : "text-slate-500 hover:text-slate-700"
                  }`}
            ...
            <div className="lexitems-center gap-2 sm:gap-4">
              <Button
