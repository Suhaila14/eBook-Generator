          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-violet-600 rounded-lg hover:bg-violet-50/50 transition-all duratin-200"
              >
                {link.name}
              </a>
            ))}
          ...
          <nav className="px-4 py-4 space-y-1">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="block px-4 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:text-violet-600 hover:bg-violet-100 transition-all duration-200"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="px-4 py-4 border-t border-gray-100">
            {isAuthenticated ? (
              ...
              <div className="space-y-2">
                <a
                  href="/login"
                  className="block text-center px-4 py-2.5 text-sm font-medium  text-black hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all duration-200"
                >
                  Login
                </a>
                ...
                <a
                  href="/signup"
                  className="block text-center px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-violet-400 to-purple-500 rounded-lg shadow-lg shadow-violet-500/30 hover:from-violet-500 hover:to-purple-600 hover:scale-105 transition-all duration-200"
                >
                  Get Started
                </a>
              </div>
