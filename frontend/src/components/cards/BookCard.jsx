...
const BookCard = ({ book, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-200">
        <img
          src={coverImageUrl}
          alt={book.title}
          className="w-full aspect-[9/13] object-cover transition-transform duration-500 group-hover:scale-105"
          //create blank image space
          onError={(e) => {
            e.target.src = "";
          }}
        />
        <div className="absolute top-3 right-3 opacity-100 group-hover:opacity-100 transition-opacity duration-200 flex gap-2">
          <button
            onClick={(e) => {
              e.stopPropagation();
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-gradient-to-r from-orange-400 via-amber-500 to-rose-800 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
    </div>
  );
};
