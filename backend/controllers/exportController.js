...
//process markdown content into docs paragraph
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      if (token.type === "heading_open") {
        ...
        fillColor(TYPOGRAPHY.colors.text).text(token.content, {
          indent: 20,
          align: "left",
        });
...
const exportAsPDF = async (req, res) => {
  try {
    ...
    const doc = new PDFDocument({
      margins: { top: 72, bottom: 70, left: 72, right: 72 },
      bufferPages: true,
      autoFirstPage: true,
    });
    ...
    if (book.coverImage && !book.coverImage.includes("pravatar")) {
      ...
      try {
        if (fs.existsSync(imagePath)) {
          ...
          const pageHeight =
            doc.page.height - doc.page.margins.left - doc.page.margins.bottom;
